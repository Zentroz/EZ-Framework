...
Shader::Shader(const char* name, const char* vertexEntryPoint, const char* pixelEntryPoint) noexcept {
	this->name = name;
...
void Shader::CompileFromFile(const char* filePath, ID3D11Device* device) {
	...
	hr = D3DCompileFromFile(wfile.c_str(), nullptr, nullptr, vertexEntryPoint, "vs_5_0", 0, 0, &vsBlob, &errBlob);

	CHECK_SHADER(hr, errBlob, name);

	hr = D3DCompileFromFile(wfile.c_str(), nullptr, nullptr, pixelEntryPoint, "ps_5_0", 0, 0, &psBlob, &errBlob);
	...
	CreateInputLayout(vsBlob, device);
...
void Shader::CompileFromSource(const char* vs_src, const char* ps_src, ID3D11Device* device) {
	...
	hr = D3DCompile(vs_src, strlen(vs_src), nullptr, nullptr, nullptr, vertexEntryPoint, "vs_5_0", 0, 0, &vsBlob, &errBlob);
	CHECK_SHADER(hr, errBlob, "Failed to compile vertex shader");

	// Compile pixel shader
	Microsoft::WRL::ComPtr<ID3DBlob> psBlob;
	hr = D3DCompile(ps_src, strlen(ps_src), nullptr, nullptr, nullptr, pixelEntryPoint, "ps_5_0", 0, 0, &psBlob, &errBlob);
	...
	CreateInputLayout(vsBlob.Get(), device);
...
void Shader::CreateInputLayout(ID3DBlob* vsBlob, ID3D11Device* device) {
	HRESULT hr;
